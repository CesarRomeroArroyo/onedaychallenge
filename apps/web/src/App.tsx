...
export function App() {
  ...
  return <main>
    ...
    {result && <>
      <section className="card dataset-card" aria-labelledby="preview-title"><div className="dataset-header"><div><p className="section-kicker">Dataset ready</p><h2 id="preview-title">{result.dataset.filename}</h2><p>{result.dataset.row_count} rows · {result.dataset.columns.length} columns · delimiter “{result.dataset.delimiter}”</p></div><button className="primary-button" disabled={busy || Boolean(analysis)} onClick={handleAnalyze}>{analysis ? "Analysis complete" : busy ? "Analyzing…" : "Analyze data"}</button></div><p className="notice">A compact sample may be sent to configured AI provider. Sensitive-looking columns are excluded. Rules always check every row.</p><div className="table-wrap"><table><thead><tr>{result.dataset.columns.map((column) => <th key={column.id}>{column.original_header || "(empty header)"}</th>)}</tr></thead><tbody>{(rows?.items ?? result.preview).map((row) => <tr key={row.id}>{result.dataset.columns.map((column) => <td key={column.id}>{row.values[column.id]}</td>)}</tr>)}</tbody></table></div></section>
